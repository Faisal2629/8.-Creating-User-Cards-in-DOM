let users = [
  ...
  // Function to create and return a user card DOM element
  function displayUserCard(user) {
    let card = document.createElement("div");
    card.classList.add("user-card"); // Add the "user-card" class for styling
  
    // Create and append elements for name, username, and email
  ...
  function displayUserCards(users) {
    ...
    users.forEach(user => {
      const card = displayUserCard(user); // Create a card for each user
      container.appendChild(card); // Append the card to the container
    });
  }
  
  // Initial display of user cards when the page loads
